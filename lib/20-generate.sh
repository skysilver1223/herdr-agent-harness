...
단, Worker와 Reviewer는 전달된 Context Packet에 SPEC 발췌와 Task 정보가 포함되어 있다면 원본을 중복해서 통독하지 않는다(Packet이 없거나 추가 정보가 필요할 때만 원본에 접근한다).
해당 Task의 Intent, 소스, 미해결 리뷰, 추가 근거 등은 필수적으로 파악한다.
