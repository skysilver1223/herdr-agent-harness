...
달라 추측하지 않고 계속 사람이 전체 모델 ID로 관리한다. 서로 다른 값 공간인
별칭과 `claude_models`를 교차 비교하거나 잔존 값 경고를 만들지도 않는다.
Provider 변화는 코드가 아니라 정책 표에 반영한다. 승인 표의
`_runtime_agent_arg_allowlist`는 그대로 닫혀 있어 승인
...
않고 둘을 교차 비교하거나 잔존 값 경고도 만들지 않는다(정책이 넓어지지도
좁아지지도 않는다). 비정상 종료·빈 목록·jq 부재(codex)는 실패로 취급해 그
Provider의 삭제를 계산하지 않는다 — Provider마다 독립적으로 판단하므로 한쪽
실패가 다른 쪽 갱신을 막지 않는다. jq 부재는 전용 상태 200, codex CLI 실행
파일 부재는 셸의 command-not-found 상태 127로 구분해 원인별 안내를 낸다.
조회 함수는 분리되어
...
이에 맞춰 진입 지침(`AGENTS.md`)도 분리됩니다. Worker와 Reviewer는 이 Packet에 포함된 내용(SPEC 발췌·Task)을 원본에서 중복 통독하지 않으며, Packet이 부재하거나 추가 정보가 필요할 때만 원본을 읽습니다. Orchestrator와 Planner는 전체 흐름 파악을 위해 `.harness/SPEC.md`와 현재 Task YAML 등의 원본을 직접 읽습니다.
